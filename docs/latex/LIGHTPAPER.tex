...
\vspace{0.3em}
\large Version 1.8 — January 2026}

\author{Sylvain Cormier\\QuantumVerse Protocols\\Paraxiom Research}
\date{}
...
\begin{quote}
\textbf{Changelog v1.8}: Updated to Public Beta status. 600+ tests passing. 3 production validators live.

\textbf{v1.7}: Clarified PQC costs—we avoid migration overhead but still have raw PQC signature size, mitigated via toroidal parallelization.

\textbf{v1.6}: Added ``Why Greenfield?'' section citing Campbell (2025) on PQC migration governance impossibility.

\textbf{v1.5}: Added Web Interface section. Added Research Publications with DOIs (6 papers on Zenodo). Expanded references.
...
\subsection{Why Greenfield?}

\textbf{Why not migrate existing chains?} Campbell (2025) demonstrates that post-quantum migration for Bitcoin and Ethereum faces what he terms ``governance impossibility'':

\begin{center}
\begin{longtable}{@{}llll@{}}
\toprule
\textbf{Impact} & \textbf{Bitcoin} & \textbf{Ethereum} \\
\midrule
Capacity loss & 50\% & 50\% \\
Fee increase & 3$\times$ & 3$\times$ \\
State bloat & 59$\times$ & 59$\times$ \\
...
\textbf{QuantumHarmony's approach}: Build post-quantum from genesis. No migration governance, no backwards compatibility debt, no hybrid transition period.

\textbf{What we avoid vs.\ what we address differently}:

\begin{center}
\begin{longtable}{@{}lll@{}}
\toprule
\textbf{Cost} & \textbf{Migration Approach} & \textbf{QuantumHarmony} \\
\midrule
Hybrid signature overhead & Required during transition & None (PQC-only from genesis) \\
Legacy state bloat & 59$\times$ for backwards compat. & None \\
Governance deadlock & Unsolvable & Not applicable \\
Raw PQC signature size & $\sim$29KB SPHINCS+ & Same ($\sim$29KB) \\
\textbf{Mitigation} & None proposed & 512-segment toroidal parallel. \\
\bottomrule
\end{longtable}
\end{center}

PQC signatures are inherently larger than classical signatures. Campbell's state bloat comes from maintaining \textit{both} formats during migration. QuantumHarmony still has larger signatures, but compensates through parallel verification across the toroidal mesh (see Section 2.4), achieving 10K TPS with SPHINCS+ signatures.
...
\textbf{Phase}: Public Beta Testnet \\
\textbf{Validators}: 3 production validators (Montreal, Beauharnois, Frankfurt) \\
\textbf{Block time}: 6 seconds \\
\textbf{Consensus}: Proof of Coherence (PQ-BFT) \\
\textbf{Test Coverage}: 600+ unit and integration tests passing

\textbf{What's implemented}:
\begin{itemize}
  \item SPHINCS+ signature verification across all pallets
  \item Falcon-1024/ML-KEM-1024 quantum P2P networking
  \item Coherence Gadget for deterministic BFT finality
  \item All governance pallets (Democracy, Treasury, Collective)
  \item Financial pallets (QCAD stablecoin, Fideicommis trusts)
  \item Legal pallets (Ricardian contracts, Notarial attestation)
  \item 512-segment toroidal mesh parallelization
  \item Docker deployment for node operators
  \item Forkless runtime upgrades
\end{itemize}

\textbf{Seeking}:
\begin{itemize}
  \item Security audit (pre-mainnet)
  \item QKD hardware production testing
  \item Additional validator operators
\end{itemize}
...
\begin{enumerate}
  ...
  \item Campbell, R. ``Hybrid Post-Quantum Signatures for Bitcoin and Ethereum: A Protocol-Level Integration Strategy.'' JBBA 9(1), 2025. DOI: 10.31585/jbba-9-1-(2)2026
\end{enumerate}
