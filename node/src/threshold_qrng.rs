...
///
/// Each QRNG device (e.g., Toshiba QKD, Crypto4A HSM, IdQuantique) produces
/// a Shamir secret share along with a quality metric (QBER) and a STARK proof
/// attesting to the authenticity of the quantum measurement.
///
/// Shares are ranked by QBER (lower = better) so that `reconstruct_entropy()`
/// always selects the K highest-quality shares for Shamir reconstruction.
#[derive(Clone, Debug, Encode, Decode, PartialEq, Eq, Hash)]
pub struct DeviceShare {
    /// Device identifier (e.g., "qkd-toshiba", "crypto4a-hsm", "idquantique")
    pub device_id: DeviceId,

    /// Shamir share bytes (produced by the `sharks` crate dealer)
    pub share: Vec<u8>,

    /// Quantum Bit Error Rate (QBER) scaled by 100 (basis points).
    ///
    /// QBER measures the error rate on the quantum channel between sender and
    /// receiver. It is the primary quality indicator for quantum entropy:
    ///
    /// - **0-200 (0-2%)**: Excellent — typical for lab-grade QKD links
    /// - **200-500 (2-5%)**: Good — production-grade HSM-based QRNG
    /// - **500-800 (5-8%)**: Acceptable — long-distance QKD or noisy channels
    /// - **800-1100 (8-11%)**: Marginal — near the BB84 security threshold
    /// - **>1100 (>11%)**: Rejected — exceeds BB84 max tolerable QBER (14.6%)
    ///
    /// The BB84 protocol guarantees security when QBER < 14.6%. QuantumHarmony
    /// uses a conservative 11% threshold (`qber_threshold` in `DeviceConfig`).
    ///
    /// Scaling: `qber = 80` means 0.80%, `qber = 1100` means 11.00%.
    pub qber: u32,

    /// STARK proof of quantum measurement authenticity (Level 1 in the 3-level
    /// proof hierarchy). Proves the entropy was generated by a genuine quantum
    /// process and not by a classical PRNG.
    pub stark_proof: Vec<u8>,

    /// Unix timestamp (seconds) when the quantum measurement was taken.
    /// Used for freshness validation — stale measurements are rejected.
    pub timestamp: u64,
}
...
/// Configuration for the K-of-M threshold QRNG system.
///
/// Uses Shamir Secret Sharing to combine entropy from multiple independent
/// quantum devices into a single Byzantine-fault-tolerant entropy source.
/// Any K out of M devices can reconstruct the secret, so the system tolerates
/// up to M-K device failures while maintaining entropy quality.
///
/// # Default Configuration
///
/// K=2, M=3 with devices: Toshiba QKD, Crypto4A HSM, IdQuantique.
/// This tolerates 1 device failure (e.g., network partition to one QRNG).
///
/// # Security Properties
///
/// - **Threshold**: At least K shares required — fewer reveals nothing
/// - **Quality-Ordered**: `reconstruct_entropy()` selects K best shares by QBER
/// - **Device Independence**: Each device uses a different quantum source
/// - **QBER Filtering**: Per-device `qber_threshold` rejects noisy channels
#[derive(Clone, Debug)]
pub struct ThresholdConfig {
    /// Minimum number of shares required for reconstruction (K).
    /// Must satisfy: 2 <= K <= M.
    pub threshold_k: u32,
    /// Total number of QRNG devices available (M).
    pub total_devices_m: u32,
    /// Per-device configuration (endpoints, QBER thresholds, enabled status).
...
///
/// Shares are sorted by QBER (ascending) and the best K are selected for
/// Lagrange interpolation via the `sharks` crate. This ensures the
/// reconstructed secret always uses the highest-quality quantum measurements.
///
/// # Arguments
/// * `device_shares` - Shares from multiple QRNG devices
/// * `threshold_k` - Minimum number of shares required (K parameter)
///
/// # Returns
/// Combined entropy bytes on success, or an error message.
///
/// # Errors
/// - `"Insufficient shares: N < K"` — fewer shares than threshold
/// - `"Share N conversion failed"` — share bytes are malformed
/// - `"Shamir reconstruction failed"` — shares may be from different secrets
///
/// # Examples
///
/// ```rust,ignore
/// use threshold_qrng::{DeviceShare, DeviceId, reconstruct_entropy};
/// use sharks::Sharks;
///
/// // Create shares from a secret using K=2
/// let secret = b"32_bytes_of_quantum_entropy_here";
/// let sharks = Sharks(2);
/// let dealer = sharks.dealer(secret);
/// let shares: Vec<_> = dealer.take(3).collect();
///
/// // Wrap as DeviceShares with QBER metadata
/// let device_shares: Vec<DeviceShare> = shares.iter().enumerate().map(|(i, s)| {
///     DeviceShare {
///         device_id: DeviceId::from_str(&format!("device-{}", i)),
///         share: Vec::from(s),
///         qber: (i as u32 + 1) * 100,  // 100, 200, 300 basis points
///         stark_proof: vec![],
///         timestamp: 0,
///     }
/// }).collect();
///
/// // Reconstruct — selects 2 best shares (lowest QBER)
/// let entropy = reconstruct_entropy(&device_shares, 2).unwrap();
/// assert_eq!(entropy, secret);
/// ```
