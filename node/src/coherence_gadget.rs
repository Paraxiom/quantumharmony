    ...
    Client::Api: sp_consensus_aura::AuraApi<Block, sp_consensus_aura::sphincs::AuthorityId>,
{
    ...
    fn get_qrng_entropy_for_election(
        ...
        if let Some(shares) = shares_opt {
            // 1. Pull the threshold K directly from our configuration
            // to stay consistent with the reconstruction math.
            let threshold_k = self
                .threshold_config
                .lock()
                .map(|c| c.threshold_k as usize)
                .unwrap_or(2);

            if shares.len() >= threshold_k {
                // 2. Call your newly implemented Shamir reconstruction logic
                match self.reconstruct_qrng_entropy(&shares) {
                    Ok(entropy) => {
                        // 3. Calculate average QBER for logging/monitoring
                        let avg_qber: f64 =
                    ...
                    Err(e) => {
                        // If math fails (e.g. invalid shares), we fall back to VRF to prevent network stall
                        warn!(
                            "⚠️  QRNG Shamir reconstruction failed: {}, falling back to VRF",
                            e
                        );
                    }
                }
                ...
                debug!(
                    "Insufficient QRNG shares for round {}: {}/{}, using VRF fallback",
                    round_id,
                    shares.len(),
                    threshold_k
                );
